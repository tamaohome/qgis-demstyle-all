...
class DataRangeSlider(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self._slider.setMaximum(len(self.DATA_RANGE_VALUES) - 1)
    ...
    def _update_label(self) -> None:
        ...
        if 0 <= index < len(self.DATA_RANGE_VALUES):
            value = self.DATA_RANGE_VALUES[index]
    ...
    def get_actual_value(self) -> int:
        ...
        index = self.value()
        if 0 <= index < len(self.DATA_RANGE_VALUES):
            return self.DATA_RANGE_VALUES[index]
        return self.DATA_RANGE_VALUES[0]
    ...
    def set_value_from_actual(self, actual_value: int) -> None:
        ...
        """
        try:
            index = self.DATA_RANGE_VALUES.index(actual_value)
            self.setValue(index)
        except ValueError:
            raise ValueError(
                f"値 {actual_value} は DATA_RANGE_VALUES に含まれていません。"
                f"使用可能な値: {self.DATA_RANGE_VALUES}"
            )
